-- Complex analytics pipeline with MERGE containing UNION, window functions, and UDF
-- SparkSQL dialect
-- This variant has a UNION inside the MERGE statement's USING clause
...
-- MERGE statement with UNION inside the USING clause
-- This combines data from multiple sources before upserting
MERGE INTO analytics.customer_lifetime_value AS target
USING (
    -- Primary source: customer summary from recent orders
    SELECT
        cs.customer_id,
        cs.customer_name,
        cs.email,
    ...
    -- Secondary source: subscription revenue
    SELECT
        s.customer_id,
        c.customer_name,
        c.email,
        COUNT(s.subscription_id) AS lifetime_orders,
        SUM(s.monthly_amount * s.months_active) AS lifetime_value,
        AVG(s.monthly_amount) AS avg_order_value,
        MIN(s.start_date) AS first_purchase_date,
        MAX(COALESCE(s.end_date, CURRENT_DATE())) AS last_purchase_date,
        classify_customer_tier(
            SUM(s.monthly_amount * s.months_active),
            COUNT(s.subscription_id)
        ) AS customer_tier,
        'subscription' AS data_source,
        CURRENT_TIMESTAMP() AS updated_at
    FROM subscriptions s
    INNER JOIN customers c ON s.customer_id = c.customer_id
    WHERE s.status = 'active' OR s.end_date > DATE_SUB(CURRENT_DATE(), 365)
    GROUP BY s.customer_id, c.customer_name, c.email

    UNION

    -- Tertiary source: partner program revenue (deduplicated with UNION)
    SELECT
        pp.customer_id,
        c.customer_name,
        c.email,
        COUNT(DISTINCT pp.transaction_id) AS lifetime_orders,
        SUM(pp.commission_amount) AS lifetime_value,
        AVG(pp.commission_amount) AS avg_order_value,
        MIN(pp.transaction_date) AS first_purchase_date,
        MAX(pp.transaction_date) AS last_purchase_date,
        CASE
            WHEN SUM(pp.commission_amount) > 50000 THEN 'platinum'
            WHEN SUM(pp.commission_amount) > 20000 THEN 'gold'
            WHEN SUM(pp.commission_amount) > 5000 THEN 'silver'
            ELSE 'bronze'
        END AS customer_tier,
        'partner_program' AS data_source,
        CURRENT_TIMESTAMP() AS updated_at
    FROM partner_program pp
    INNER JOIN customers c ON pp.customer_id = c.customer_id
    WHERE pp.status = 'completed'
    GROUP BY pp.customer_id, c.customer_name, c.email
    HAVING SUM(pp.commission_amount) > 1000
) AS source
...
WHEN MATCHED AND source.lifetime_value > target.lifetime_value THEN
    UPDATE SET
        ...
        target.lifetime_orders = target.lifetime_orders + source.lifetime_orders,
        target.lifetime_value = GREATEST(target.lifetime_value, source.lifetime_value),
        target.avg_order_value = (target.avg_order_value + source.avg_order_value) / 2,
        target.first_purchase_date = LEAST(target.first_purchase_date, source.first_purchase_date),
        target.last_purchase_date = GREATEST(target.last_purchase_date, source.last_purchase_date),
        target.customer_tier = CASE
            WHEN source.customer_tier = 'platinum' OR target.customer_tier = 'platinum' THEN 'platinum'
            WHEN source.customer_tier = 'gold' OR target.customer_tier = 'gold' THEN 'gold'
            WHEN source.customer_tier = 'silver' OR target.customer_tier = 'silver' THEN 'silver'
            ELSE 'bronze'
        END,
...
WHEN NOT MATCHED BY SOURCE AND target.updated_at < DATE_SUB(CURRENT_DATE(), 730) THEN
    DELETE;
